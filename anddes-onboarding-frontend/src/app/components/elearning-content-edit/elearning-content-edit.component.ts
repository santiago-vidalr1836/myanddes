...
export class ElearningContentEditComponent implements OnInit {
  ...
  back() {
    if (this.content.id == undefined) {
      alert("No se han guardado los cambios. Si regresa ahora se perderán");
      this.location.back()
    } else {
      this.location.back();
    }
  ...
  save() {
    ...
    if (this.content.id == undefined) {
      this.activityService.saveELearningContent(Constants.ACTIVITY_INDUCTION_ELEARNING, this.content).subscribe(() => {
        this.snackBarService.success('Contenido creado')
