...
class _FlutterFlowVideoPlayerState extends State<FlutterVideoPlayer>
    with RouteAware {
  ...
  Future _initializePlayer() async {
    ...
    _videoPlayerController!.addListener(() {
      if (_videoPlayerController!.value.hasError && !_loggedError) {
        log('Error playing video: ${_videoPlayerController!.value.errorDescription}');
        _loggedError = true;
      }
      // Stop all other players when one video is playing.
      if(_videoPlayerController!.value.position == _videoPlayerController!.value.duration){
        log('Finished Video ${_videoPlayerController!.value.position}');
      }
      if(_videoPlayerController!.value.isCompleted){
        log('Finished video isCompleted');
      }
